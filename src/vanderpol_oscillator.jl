...
function load_parameters()
    ...
    p["F_crit"] = 2         # [N] results from the bifurcation analysis (perform it a bit cleaner!).
...
function vdP(u, p, t)
    ...
    if norm(u, 2) > 10.0
        ...
        return -[du1, du2]       # unstable limit cycle --> negative sign.
    end
end
...
function solve_vdP(u₀, tspan, p)
    ...
    if p["fixed_dt"]
        return solve(ivp, Vern7(), dt = p["dt"], adaptive = false)
    else
        return solve(ivp, Vern7(), dense = true)
    end
end
