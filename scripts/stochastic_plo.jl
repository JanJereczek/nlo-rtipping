...
function main()
    ...
    nF, na = 30, 20                         # Number of points sampled in the ramp-parameter space.
    a_llim, a_ulim = 0, 2                   # Range of sampled slopes.
    Δx = p["Δx"]                            # Standard value if Δx not changed over looping.-
    F_llim, F_ulim = -10, 0                  # linscale with ref = F_crit
    ...
    grid_dict = Dict()
    nm = 1
    grid_file = string("EM_grid_tbuffer", p["t_buffer"], "_dt", p["dt"], "_nm", nm, ".jld2")

    if compute_grid_bool
        for σ in σ_vec
            grid_dict[string(σ)] = threaded_stochastic_scatter(σ, sss, solve_stochastic_plo, nm)
        end
